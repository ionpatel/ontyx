...
function generateInvoicePDFWithLogo(data: InvoicePDFData, logoData: { data: string; format: string } | null): jsPDF {
  ...
  // Header row: Company name (left) | Invoice details (right)
  const headerStartY = y
  
  // Left side: Company name or INVOICE title
  if (logoAdded) {
    // Logo already shown, just add company name below it
    doc.setFontSize(16)
  ...
  // Right side: Invoice details (aligned with company name)
  ...
  // Company address details (below company name)
  doc.setFontSize(9)
  doc.setTextColor(COLORS.textMuted)
  doc.setFont(FONTS.regular, 'normal')
  ...
  doc.setFontSize(7)
  doc.text(
    'Generated by Ontyx ERP • ontyx.ca',
    pageWidth / 2,
    pageHeight - 10,
    { align: 'center' }
  )
